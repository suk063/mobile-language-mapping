...
def train(cfg: TrainConfig):
    ...
    agent.load_state_dict(torch.load('pre-trained/prepare_groceries/latest_agent.pt', map_location=device), strict=False)

    agent.static_map = static_map
    agent.implicit_decoder = implicit_decoder

    for module, ckpt in [
        (agent.static_map,      "pre-trained/prepare_groceries/13/latest_static_map.pt"),
        (agent.implicit_decoder,"pre-trained/prepare_groceries/13/latest_decoder.pt")]:
        try:
