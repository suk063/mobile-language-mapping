...
GROUP=$TASK-map_prepare_groceries-$SUBTASK
EXP_NAME="$ENV_ID/$GROUP/bc-$SUBTASK-$OBJ-map_prepare_groceries-trajs_per_obj=$TRAJS_PER_OBJ"
# shellcheck disable=SC2001
PROJECT_NAME="map_prepare_groceries"
...
if [ -f "$RESUME_CONFIG" ] && [ -f "$RESUME_LOGDIR/models/latest.pt" ]; then
    echo "RESUMING"
    SAPIEN_NO_DISPLAY=1 python -m experiment.002_train_prepare_groceries "$RESUME_CONFIG" RESUME_LOGDIR="$RESUME_LOGDIR" \
    ...
    echo "STARTING"
    SAPIEN_NO_DISPLAY=1 python -m experiment.002_train_prepare_groceries configs/002_train_prepare_groceries.yml \
