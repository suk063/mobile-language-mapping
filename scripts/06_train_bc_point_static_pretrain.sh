...
if [ -f "$RESUME_CONFIG" ] && [ -f "$RESUME_LOGDIR/models/latest.pt" ]; then
    echo "RESUMING"
    SAPIEN_NO_DISPLAY=1 python -m training_script.train_bc_point_static_pretrain "$RESUME_CONFIG" RESUME_LOGDIR="$RESUME_LOGDIR" \
    ...
    echo "STARTING"
    SAPIEN_NO_DISPLAY=1 python -m training_script.train_bc_point_static_pretrain configs/bc_pick_static.yml \
