...
if [ -f "$RESUME_CONFIG" ] && [ -f "$RESUME_LOGDIR/models/latest.pt" ]; then
    echo "RESUMING"
    SAPIEN_NO_DISPLAY=1 python -m training_script.17_train_bc_point_global_gridnet_multiepisode_dit "$RESUME_CONFIG" RESUME_LOGDIR="$RESUME_LOGDIR" \
    ...
    echo "STARTING"
    SAPIEN_NO_DISPLAY=1 python -m training_script.17_train_bc_point_global_gridnet_multiepisode_dit configs/16_bc_pick_global_gridnet_multiepisode_policy_pretrained.yml \
