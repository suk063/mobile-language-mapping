...
EXP_NAME="$ENV_ID/$GROUP/bc-$SUBTASK-$OBJ-static-global-local-trajs_per_obj=$TRAJS_PER_OBJ"
# shellcheck disable=SC2001
...
if [ -f "$RESUME_CONFIG" ] && [ -f "$RESUME_LOGDIR/models/latest.pt" ]; then
    echo "RESUMING"
    SAPIEN_NO_DISPLAY=1 python -m training_script.16_train_bc_point_global_gridnet_multiepisode_policy_pretrained "$RESUME_CONFIG" RESUME_LOGDIR="$RESUME_LOGDIR" \
    ...
    echo "STARTING"
    SAPIEN_NO_DISPLAY=1 python -m training_script.16_train_bc_point_global_gridnet_multiepisode_policy_pretrained configs/16_bc_pick_global_gridnet_multiepisode_policy_pretrained.yml \
