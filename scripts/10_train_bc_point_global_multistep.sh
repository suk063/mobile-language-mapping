...
if [ -f "$RESUME_CONFIG" ] && [ -f "$RESUME_LOGDIR/models/latest.pt" ]; then
    echo "RESUMING"
    SAPIEN_NO_DISPLAY=1 python -m training_script.10_train_bc_point_global_multistep "$RESUME_CONFIG" RESUME_LOGDIR="$RESUME_LOGDIR" \
    ...
    echo "STARTING"
    SAPIEN_NO_DISPLAY=1 python -m training_script.10_train_bc_point_global_multistep configs/10_bc_pick_global_multistep.yml \
