...
MAX_CACHE_SIZE=170_000   # safe num for about 64 GiB system memory
...
if [ -f "$RESUME_CONFIG" ] && [ -f "$RESUME_LOGDIR/models/latest.pt" ]; then
    echo "RESUMING"
    SAPIEN_NO_DISPLAY=1 python -m training_script.train_bc_point_dynamic_flow "$RESUME_CONFIG" RESUME_LOGDIR="$RESUME_LOGDIR" \
