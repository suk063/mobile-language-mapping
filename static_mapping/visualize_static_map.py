...
from lang_mapping.mapper.mapper import MultiVoxelHashTable, VoxelHashTable
...
class App:
    def __init__(self, cfg: AppConfig):
        ...
        if len(self.dataset.scene_ids) != self.cfg.grid_cfg.n_scenes:
            logger.warning(
        ...
        # Create model and load grid net
        if self.cfg.grid_cfg.name == "voxel_hash_table":
            ...
            state_fp = os.path.join(self.cfg.test_model_dir, "hash_voxel_sparse.pt")
            print(f"Loading VoxelHashTable state from {state_fp}")
            if self.cfg.grid_cfg.n_scenes > 1:
                self.grid_net = MultiVoxelHashTable.load_sparse(state_fp)
                self.grid_net.distribute_to_devices()
            else:
                self.grid_net = VoxelHashTable.load_sparse(state_fp)
                self.grid_net.to(self.cfg.device_decoder)
        ...
        if self.current_grid_map < 0 or self.current_grid_map >= self.cfg.grid_cfg.n_scenes:
            raise ValueError(
                f"Invalid initial grid map ID: {self.current_grid_map}. "
                f"Must be in range [0, {self.cfg.grid_cfg.n_scenes - 1}]."
            )
        self._update_pcd()
    ...
    def _next_grid_map(self, vis, action, mod):
        ...
        if action != 0:  # Only handle key press events
            return False
        if self.current_grid_map < self.cfg.grid_cfg.n_scenes - 1:
            self.current_grid_map += 1
            self._update_display()
            print(f"Grid map: {self.current_grid_map + 1}/{self.cfg.grid_cfg.n_scenes}")
        else:
    ...
    def _collect_vertices(self):
        ...
        print(
            ...
            f"grid map {self.current_grid_map + 1}/{self.cfg.grid_cfg.n_scenes}"
        )
    ...
    def _forward_model(self, vertices: torch.Tensor):
        ...
        with torch.no_grad():
            for i in tqdm(range(0, len(vertices), self.app_cfg.batch_size), desc="Forward Model", ncols=80):
                ...
                if isinstance(self.grid_net, (GridNet, MultiVoxelHashTable)):
                    ...
                    batch_map_features = self.grid_net.query_voxel_feature(batch_vertices)
    ...
    def _update_pcd(self):
        ...
        color_pcd_fp = os.path.join(test_model_dir, f"color_{self.current_grid_map}.ply")
        grid_map_pcd_fp = os.path.join(test_model_dir, f"grid_map_{self.current_grid_map}.ply")
        decoder_features_pcd_fp = os.path.join(test_model_dir, f"decoder_features_{self.current_grid_map}.ply")
        if os.path.exists(color_pcd_fp) and os.path.exists(grid_map_pcd_fp) and os.path.exists(decoder_features_pcd_fp):
            print(f"Loading existing point clouds for grid map {self.current_grid_map}...")
            pcd_color = o3d.io.read_point_cloud(color_pcd_fp)
            self.pcd_color.points = pcd_color.points
            self.pcd_color.colors = pcd_color.colors

            pcd_map = o3d.io.read_point_cloud(grid_map_pcd_fp)
            self.pcd_map.points = pcd_map.points
            self.pcd_map.colors = pcd_map.colors

            pcd_decoder = o3d.io.read_point_cloud(decoder_features_pcd_fp)
            self.pcd_decoder.points = pcd_decoder.points
            self.pcd_decoder.colors = pcd_decoder.colors
        else:
            ...
            print(
                f"Collected {len(vertices)} vertices for grid map {self.current_grid_map + 1}/{self.cfg.grid_cfg.n_scenes}"
            )
            ...
            o3d.io.write_point_cloud(color_pcd_fp, self.pcd_color)
            print(f"Saved color point cloud to {color_pcd_fp}")
            ...
            o3d.io.write_point_cloud(grid_map_pcd_fp, self.pcd_map)
            print(f"Saved grid map point cloud to {grid_map_pcd_fp}")
            ...
            o3d.io.write_point_cloud(decoder_features_pcd_fp, self.pcd_decoder)
            print(f"Saved decoder features point cloud to {decoder_features_pcd_fp}")
