...
GROUP=$TASK-$SUBTASK
EXP_NAME="$ENV_ID/$GROUP/$TASK-$SUBTASK-image-$SEED"
# shellcheck disable=SC2001
PROJECT_NAME="$TASK-$SUBTASK-image"
...
if [ -f "$RESUME_CONFIG" ] && [ -f "$RESUME_LOGDIR/models/latest.pt" ]; then
    ...
    echo "STARTING"
    SAPIEN_NO_DISPLAY=1 python -m experiment.train_bc configs/image/train_${REPRESENTATION}_${TASK}.yml \
