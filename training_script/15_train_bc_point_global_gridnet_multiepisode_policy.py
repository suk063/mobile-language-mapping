...
def load_logged_centers(root_dir: str = "queried_cell_centers"):
    """
    Load every saved voxel‑center file in *root_dir* and return a
    dict {level: centers_np_array}.

    Each file is expected to follow the naming convention
    'level{l}_centers.npy' that the training script uses.
    """
    centers_by_level = {}
    pattern = os.path.join(root_dir, "level*_centers.npy")

    for fp in glob.glob(pattern):
        # e.g. 'level2_centers.npy'  ->  2
        lvl = int(os.path.basename(fp).split("_")[0].lstrip("level"))
        centers_by_level[lvl] = np.load(fp)         # shape (M_l, 3), dtype float32

    if not centers_by_level:
        raise FileNotFoundError(f"No voxel‑center files found in {root_dir}")
...
def train(cfg: TrainConfig):
    ...
    # static_map = GridNet(cfg=asdict(cfg.algo.grid_cfg), device='cpu', dtype=torch.float16)
