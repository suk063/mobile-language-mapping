...
from lang_mapping.agent.agent_static_target import Agent_static_target
...
def get_translation_from_subtask_poses(
    subtask_poses: Dict[str, Dict[str, Dict[str, List[List[float]]]]],
    subtask_uids: List[str]
) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    Extracts (x, y, z) translations from 4x4 pose matrices for given subtask_uids.

    Args:
        subtask_poses (Dict): Pose data structured as from load_target_poses_from_plan().
        subtask_uids (List[str]): List of subtask UIDs for the current batch.

    Returns:
        targets_translation (torch.Tensor): Tensor of shape [batch_size, 3]
        rigid_objs_translation (torch.Tensor): Tensor of shape [batch_size, 3]
    """

    targets_list = []
    rigid_objs_list = []

    for subtask_uid in subtask_uids:
        # Check subtask_uid existence
        if subtask_uid not in subtask_poses:
            raise KeyError(f"'{subtask_uid}' not found in subtask_poses.")

        # Extract first 4x4 pose matrix from targets
        targets_dict = subtask_poses[subtask_uid]["targets"]
        if not targets_dict:
            raise ValueError(f"'targets' for '{subtask_uid}' is empty.")

        # Extract first 4x4 pose matrix from rigid_objs
        rigid_objs_dict = subtask_poses[subtask_uid]["rigid_objs"]
        if not rigid_objs_dict:
            raise ValueError(f"'rigid_objs' for '{subtask_uid}' is empty.")

        # Get translation vectors from the pose matrices
        mat_targets = np.array(next(iter(targets_dict.values())), dtype=np.float32)
        mat_rigid = np.array(next(iter(rigid_objs_dict.values())), dtype=np.float32)

        trans_targets = mat_targets[:3, 3]
        trans_rigid = mat_rigid[:3, 3]

        targets_list.append(trans_targets)
        rigid_objs_list.append(trans_rigid)

    # Convert lists to tensors
    targets_translation = torch.from_numpy(np.stack(targets_list, axis=0))
    rigid_objs_translation = torch.from_numpy(np.stack(rigid_objs_list, axis=0))

    return targets_translation, rigid_objs_translation


def load_target_poses_from_plan(main_json_path: str, base_dir: str):
    """
    Loads a 4x4 pose for each subtask UID from a plan JSON and corresponding episode JSONs.
    Then applies a transformation to convert RCAD poses into the poses used in our scenes:
        pose_final = obj_transform * pose_original * world_transform
    Returns {subtask_uid: transformed_pose_4x4, ...}.
    """

   # Define transforms once
    q = transforms3d.quaternions.axangle2quat(np.array([1, 0, 0]), theta=np.deg2rad(90))
    world_transform = sapien.Pose(q=q).inv()          # Inverse of X-90 rotation
    obj_transform = sapien.Pose(q=q, p=[0, 0, 0])  # X-90 plus a small translation

    with open(main_json_path, 'r') as f:
        plan_data = json.load(f)

    # Final dictionary to return
    subtask_initial_poses = {}

    for plan in plan_data["plans"]:
        init_config_name = plan["init_config_name"]
        episode_path = os.path.join(base_dir, init_config_name)

        with open(episode_path, 'r') as ef:
            episode_data = json.load(ef)

        object_labels = episode_data["info"]["object_labels"]
        targets = episode_data.get("targets", {})
        rigid_objs = episode_data.get("rigid_objs", [])

        # Build a map from base object id -> possible episode target keys
        # e.g. "024_bowl" -> ["024_bowl_:0001"]
        label_map = {}
        for k in object_labels.keys():
            base_name = k.split('_:')[0]
            label_map.setdefault(base_name, []).append(k)

        for subtask in plan["subtasks"]:
            subtask_uid = subtask["uid"]
            obj_id = subtask["obj_id"]
            base_obj_id = obj_id.rsplit('-', 1)[0]

            # Prepare an entry for each subtask
            subtask_initial_poses[subtask_uid] = {"targets": {}, "rigid_objs": {}}

            # ---- Part 1: "targets" pose ----
            if base_obj_id in label_map:
                ep_key = label_map[base_obj_id][0]  # e.g. "024_bowl_:0001"
                if ep_key in targets:
                    pose_4x4 = targets[ep_key]
                    if pose_4x4 is None:
                        raise ValueError(f"Pose is None for subtask {subtask_uid}")
                    mat = np.array(pose_4x4, dtype=float)
                    original_pose = sapien.Pose(mat)
                    transformed_pose = obj_transform * original_pose * world_transform
                    subtask_initial_poses[subtask_uid]["targets"][ep_key] = transformed_pose.to_transformation_matrix().tolist()
                else:
                    raise KeyError(f"Key '{ep_key}' not found in targets for subtask {subtask_uid}.")
            else:
                raise KeyError(f"Object ID '{base_obj_id}' not found in label_map for subtask {subtask_uid}.")

            # ---- Part 2: "rigid_objs" pose ----
            # We look for something like "024_bowl.object_config.json"
            found_rigid_obj = False
            for rigid_obj_name, rigid_pose_4x4 in rigid_objs:
                # If the rigid object's filename includes the base_obj_id, assume it's the match
                if base_obj_id in rigid_obj_name:
                    mat = np.array(rigid_pose_4x4, dtype=float)
                    original_pose = sapien.Pose(mat)
                    transformed_pose = obj_transform * original_pose * world_transform
                    subtask_initial_poses[subtask_uid]["rigid_objs"][rigid_obj_name] = transformed_pose.to_transformation_matrix().tolist()
                    found_rigid_obj = True
                    break

            if not found_rigid_obj:
                raise KeyError(
                    f"No matching rigid object found for base_obj_id '{base_obj_id}' in subtask {subtask_uid}."
                )

    return subtask_initial_poses
...
def train(cfg: TrainConfig):
    ...
    base_dir = "/home/sunghwan/.maniskill/data/scene_datasets/replica_cad_dataset/rearrange"  # episode_*.json 파일들이 있는 상위 경로
    subtask_poses = load_target_poses_from_plan(cfg.eval_env.task_plan_fp, base_dir)
    ...
    for epoch in range(cfg.algo.stage2_epochs):
        ...
        for obs, act, subtask_uids, step_nums in tqdm(bc_dataloader, desc="Stage2-Batch", unit="batch"):
            ...
            targets_pos, rigid_objs_pos = get_translation_from_subtask_poses(subtask_poses, subtask_uids)
            
            targets_pos = targets_pos.to(device)
            rigid_objs_pos = rigid_objs_pos.to(device)    

            pi = agent.forward_policy(obs, subtask_labels, step_nums, targets_pos, rigid_objs_pos)
