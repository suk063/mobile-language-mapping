...
def task_plan_idxs_from_uids(vec_env, allowed_uids, *, device=None):
    """
    vec_env : ManiSkillVectorEnv
    allowed_uids : set[str]
    return     : (K,) torch.LongTensor,  K = #task-plans containing at least one UID
    """
    if device is None:
        device = torch.device("cpu")

    # 1) 실제 SequentialTaskEnv 찾기
    inner_env = getattr(vec_env, "unwrapped", vec_env)
    if not hasattr(inner_env, "base_task_plans"):
        inner_env = vec_env.envs[0]                # gymnasium VectorEnv fallback
    ...
    # 3) 패딩·타일링 없이 그대로 반환
    return torch.as_tensor(valid_idxs, dtype=torch.long, device=device)
...
def train(cfg: TrainConfig):
    ...
    episode_names = sorted(episode2subtasks)         
    random.seed(cfg.seed)                         
