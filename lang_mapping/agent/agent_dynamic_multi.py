...
class Agent_point_dynamic_multi(nn.Module):
    def __init__(
        self,
        sample_obs,
        single_act_shape,
    ...
    def forward(self, observations, object_labels, step_nums):
        ...
        head_pose_expanded = head_pose.expand(-1, N, -1, -1).reshape(B_*N, -1)

        state_expanded = state.unsqueeze(1).expand(-1, N, -1).reshape(B_*N, -1)

        # Query voxel for hand
