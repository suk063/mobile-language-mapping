...
class Agent_global_gridnet_multiepisode_dit(nn.Module):
    ...
    def forward_policy(self, observations, object_labels, batch_episode_ids):
        ...
        # kv_feats = self.voxel_proj(kv_feats.view(-1, C)).view(B, L, -1)
        pts_kv   = torch.cat([kv_coords, kv_feats], dim=-1)            # [B,L, 3+768]
        
        global_coords, global_tok = self.scene_encoder(pts_kv, kv_pad)     
        # coords_vis = torch.cat([coords_hand_t, coords_head_t], dim=1)        
        # visual_tok, global_tok = self.bica(
        #     visual_tok, global_tok, coords_vis, global_coords
        # )
