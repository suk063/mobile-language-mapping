...
class Agent_static_dino(nn.Module):
    def __init__(
        self,
        sample_obs,
        single_act_shape,
        ...
        self.dino_dim_reducer = nn.Linear(dino_input_dim, voxel_feature_dim).to(self.device)
    ...
    def forward_mapping(self, observations):
        ...
        with torch.no_grad():
            ...
            head_visfeat_dino = get_visual_features_dino(self.dino_model, head_rgb)
        ...
        with torch.no_grad():
            ...
            head_visfeat_dino = head_visfeat_dino.permute(0, 2, 3, 1).reshape(B_, N, -1)
        feats_hand_dino_flat = hand_visfeat_dino.reshape(B_ * N, -1)  # (B*N, 1024)
        ...
        # ========== Implicit decode & compute cos sim ==========
    ...
    def forward_policy(self, observations, object_labels, step_nums):
        ...
        hand_visfeat_dino = hand_visfeat_dino.permute(0, 2, 3, 1).reshape(B_, N, -1)
        head_visfeat_dino = head_visfeat_dino.permute(0, 2, 3, 1).reshape(B_, N, -1)
        feats_hand_dino_flat = hand_visfeat_dino.reshape(B_ * N, -1)
        feats_head_dino_flat = head_visfeat_dino.reshape(B_ * N, -1)
