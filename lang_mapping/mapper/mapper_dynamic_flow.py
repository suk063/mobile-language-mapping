...
class VoxelHashTableDynamicFlow(nn.Module):
    ...
    def __init__(
        ...
        self.flow_features = nn.Parameter(
    ...
    def query_voxel_feature(self, query_pts, query_times, return_indices=False):
        ...
        if valid_mask.any():
            ...
            feats = self.features[v_idx]
            time_emb = self.time_embeddings[t_idx]

            # Fuse dynamic + time
            cond_dynamic = feats * time_emb
            feats = cond_dynamic
    ...
    def query_voxel_flow_feature(self, query_pts, query_times):
        ...
        if valid_mask.any():
            ...
            # flow embeddings
            flow_feats = self.flow_features[v_idx]
            time_flow = self.time_embeddings_flow[t_idx]

            # time + dynamic flow
            cond_flow_dyn = flow_feats * time_flow

            flow_feats = cond_flow_dyn
